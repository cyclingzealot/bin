...
echo Waiting until the load is below threshold of 4
~/bin/loadBelowCheck.bash -w -r -t=4
...
while [ $numPacks -gt 0 ] && [ -a $continueFlag ] ; do
	...
	for pack in `cat /tmp/upgradePackageList.txt | head -n 20 ` ; do
	    ...
	    set -x
		if [ -a $continueFlag ] ; then ~/bin/loadBelowCheck.bash -w -r -t=4; sudo nice -n 19 apt-get install $pack --only-upgrade --yes  -d; fi
		if [ -a $continueFlag ] ; then ~/bin/loadBelowCheck.bash -w -r -t=4; sudo nice -n 19 apt-get install $pack --only-upgrade --yes ;  fi
		echo; echo; echo
	done
